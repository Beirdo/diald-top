...
int
parse_incoming(char *line, int len, port_queue_t * curr)
{
    char            outline[1024];
    ...
    } else if (!strncasecmp(line, "monitor ", 8)) {
	if (!(curr->port.flag & FLAG_MONITOR)) {
	    curr->port.flag |= FLAG_MONITOR;
	    monitorcount++;
	...
	close_port(&curr->port);
	return (1);
    } else if (curr->port.flag & FLAG_WRITE) {
